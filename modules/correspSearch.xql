...
declare variable $cs:schema := 'https://raw.githubusercontent.com/TEI-Correspondence-SIG/CMIF/master/schema/cmi-customization.rng';
...
declare variable $cs:p_publication := cs:sanitize(request:get-parameter('publication', ''));
...
declare variable $cs:p_collection := cs:sanitize(request:get-parameter('collection', ()));
...
(: PUBLICATION :)
declare variable $cs:publication :=
    if ($cs:p_publication)
    then ("[substring-after(@*:source, '#')=$cs:p_publication]")
    else ();    

declare variable $cs:collections :=
    if ($cs:p_collection='wsp')
    then
    ('l60b87ed-3466-4019-b31a-1aad6463239f',
    'b0a5f149-1e9a-4748-81e0-119087d11450',
    'ac4c55df-a916-406e-b3f8-f6170695861f',
    'm23834e7-17eb-473a-8349-69bb665e7a23',
    'f03ae427-c314-46b9-82ff-e65923a2eab4',
    'f6460573-60d3-4f21-9f9b-62535ef2c49a',
    'AVH_AMERIKA',
    'e1e7187d-050f-44aa-9cdc-f60aaaa998d0',
    'ddc11d11-a0af-41aa-828c-f9c8613f76b2',
    'ea3dfdba-7f78-4147-85ba-c9175e25ba99',
    'g9c57d67-bba7-48d6-b125-882d87dabeb7'
    )
   else ();

(: WSP :)
declare variable $cs:collection :=
    if ($cs:p_collection='wsp')
    then ("[substring-after(@*:source, '#')=$cs:collections]")
    else ();
...
declare variable $cs:completeSearchQuery := 
    if ($cs:allowedQuery)
    then (concat('collection($cs:beacons)//tei:correspDesc[@*:source]', $cs:collection, $cs:cmiFile, $cs:publication, string-join($cs:searchQueryCorrespondent), $cs:searchQueryPlace, $cs:searchQueryDate, $cs:onlyOnlineAvailable))
    else ();