...
(:~ 
 : An implementation of XQuery 3.1's fn:json-to-xml and fn:xml-to-json functions for eXist, which does not support them natively as of 3.4.0.
 : 
...
declare function jx:xml-to-json($input as node()?) as xs:string? {
    jx:xml-to-json($input, map { "indent": "false" } )
};
...
declare function jx:xml-to-json($input as node()?, $options as map(*)) as xs:string? {
    ...
    let $serialization-parameters := 
        <output:serialization-parameters>
            ...
            <output:indent>{$options?indent}</output:indent>
        </output:serialization-parameters>
    return
    ...
    return
        typeswitch ($node)
            case element(fn:map) return
                if ($node/@key) then
                    map { $node/@key := jx:xml-to-json-recurse($node/node()) }
                else
                    jx:xml-to-json($node/node())
